...
### v1.2: fifth check (disk) + log rotation

- **Added `disk` check** (`scripts/check-disk.sh`, `disk_check` in `common.sh`): `df -kP` on `WATCHDOG_DISK_PATH` (default `/`), fails below `WATCHDOG_DISK_MIN_AVAILABLE_PCT` (default 10%). Mirrors `memory_check`'s shape.
- **`watchdog.log` had no rotation set up at all** — it was documented in the README as "rotated via logrotate," but that was aspirational and never actually implemented, so it had been growing unbounded since the initial rollout. Added `deploy/watchdog.logrotate` (daily, 14 kept, compressed after the first day) and had `scripts/deploy.sh` install it to `/etc/logrotate.d/watchdog`. No `copytruncate` needed since `run-checks.js` only holds the file open for the duration of one `appendFileSync` call every 5 minutes, never continuously.
